...
You should see `Logged in as @…` and `build=shop-v6` in the window.
...
3. In Telegram send **`/version`** — you should see **`shop-v6`** and the full path to the `bot.py` that is actually running.
...
6. **One replica only** — Telegram allows one long-poll per bot token. Do not scale this service to multiple instances. On Railway the local **single-instance port lock** is skipped automatically when **`RAILWAY_ENVIRONMENT`** is set; you can also set **`SKIP_SINGLE_INSTANCE=1`** if needed.
